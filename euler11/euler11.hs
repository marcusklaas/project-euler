...
addSlice buf resultList
    ...
    | otherwise       = addSlice (drop 1 buf) (newSlice:resultList)
        where newSlice = (head buf):(init . head $ resultList)
...
counterDiagonals = diagonals . reverse

allSlices :: Int -> [[a]] -> [[a]]
allSlices sliceLength mainGrid = concat $ map (slicesOfLength sliceLength) rowList
